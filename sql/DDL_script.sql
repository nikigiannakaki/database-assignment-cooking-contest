...
CREATE TABLE `contestant` (
  ...
  KEY `idx_contestant_episode_id` (`episode_id`),
  KEY `cook_fk4` (`cook_id`),
  KEY `idx_contestant_recipe_name` (`recipe_name`),
  ...
  CONSTRAINT `idx_contestant_episode_id` FOREIGN KEY (`episode_id`) REFERENCES `episodes` (`episode_id`) ON UPDATE CASCADE,
  CONSTRAINT `national_cuisine_fk1` FOREIGN KEY (`national_cuisine`) REFERENCES `national_cuisine` (`cuisine_name`) ON UPDATE CASCADE,
  CONSTRAINT `idx_contestant_recipe_name` FOREIGN KEY (`recipe_name`) REFERENCES `recipes` (`recipe_name`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
